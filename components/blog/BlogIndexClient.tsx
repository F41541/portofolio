      ...
      <Container>
        ...
        <div className="max-w-3xl mb-12">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-xs font-mono text-text-muted flex items-center gap-1">
              ...
              {posts.length} Publikasi Artikel
            </span>
          </div>
          ...
          <p className="mt-4 text-base md:text-lg text-text-secondary leading-relaxed">
            Pembahasan arsitektur web mendalam, optimasi performa Core Web Vitals, best practices Laravel &amp; Vue/React, serta studi kasus teknis.
          </p>
        </div>
        ...
        <div className="mb-10 p-6 rounded-2xl border border-border-subtle bg-surface-card/60 backdrop-blur-md space-y-5">
          ...
          <div className="relative">
            ...
            <Input
              type="text"
              placeholder="Cari artikel berdasarkan judul, deskripsi, atau topik..."
              value={searchQuery}
        ...
        <div className="flex items-center justify-between mb-6 text-xs text-text-muted">
          <span>
            Menampilkan <strong className="text-text-primary">{filteredPosts.length}</strong> dari {posts.length} artikel
            {selectedTag && (
              <> topik <span className="text-accent-emerald font-mono">#{selectedTag}</span></>
            )}
            {searchQuery && (
        ...
        {filteredPosts.length > 0 ? (
          ...
          <div className="py-16 text-center rounded-2xl border border-dashed border-border-subtle bg-surface-card/30">
            ...
            <h3 className="text-lg font-semibold text-text-primary">Tidak ada artikel ditemukan</h3>
